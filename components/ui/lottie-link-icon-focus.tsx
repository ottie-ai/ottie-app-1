...
interface LottieLinkIconFocusProps {
  className?: string
  size?: number
}

/**
 * Link Icon Component with focus animation
 * Uses Lottie animation that triggers on input focus
 */
export function LottieLinkIconFocus({ className = '', size = 18 }: LottieLinkIconFocusProps) {
  ...
  // Detect focus on sibling input element and disabled state
  useEffect(() => {
  ...
  const themedAnimationData = useMemo(() => {
    if (!linkAnimation) return linkAnimation

    const cloned = JSON.parse(JSON.stringify(linkAnimation))
  ...
  useEffect(() => {
    ...
    const totalFrames = linkAnimation?.op || 60
  ...
  // Calculate opacity based on dark mode and disabled state
  const opacity = useMemo(() => {
    if (isDisabled) {
      return 0.5 // Faded when disabled
    }
    return isDark ? 0.85 : 1
  }, [isDark, isDisabled])
