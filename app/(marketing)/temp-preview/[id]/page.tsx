...
function PreviewContent() {
  ...
  const handleConvertRawHtmlToMarkdown = async () => {
    if (!previewId) return
    
    setConvertingRawToMarkdown(true)
    try {
      const result = await convertRawHtmlToMarkdown(previewId)
      
      if ('error' in result) {
        setError(result.error || 'Failed to convert raw HTML to markdown')
        setConvertingRawToMarkdown(false)
        return
      }
      ...
      setError('Failed to convert raw HTML to markdown')
    } finally {
      setConvertingRawToMarkdown(false)
    }
  }
    ...
    <div className="dark bg-[#08000d] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="space-y-6">
          ...
          {preview?.raw_html && (
            <div className="border border-white/10 rounded-lg bg-white/[0.02] overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-white/[0.02]">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    onClick={handleConvertRawHtmlToMarkdown}
                    disabled={convertingRawToMarkdown}
                    variant="ghost"
                    size="sm"
                    className="text-white/60 hover:text-white hover:bg-white/10"
                  >
                    {convertingRawToMarkdown ? (
                      <>
                        <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                        Converting...
                      </>
          ...
          {/* Raw HTML Markdown Display (if converted) */}
            ...
            <div className="border border-white/10 rounded-lg bg-white/[0.02] overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-white/[0.02]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/40">
                    ({(preview.ai_ready_data.raw_html_markdown.length / 1024).toFixed(1)} KB)
                  </span>
                </div>
                <Button
                  onClick={() => {
                    navigator.clipboard.writeText(preview.ai_ready_data.raw_html_markdown)
                    setCopiedSection('raw-markdown')
                    setCopied(true)
                    setTimeout(() => {
                      setCopied(false)
                      setCopiedSection(null)
                  ...
                  className="text-white/60 hover:text-white hover:bg-white/10"
                >
                  {copied && copiedSection === 'raw-markdown' ? (
                    <>
                      <Check className="h-4 w-4 mr-2" />
                      Copied!
                    </>
